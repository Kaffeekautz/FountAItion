...
export function Header({ title, subtitle, selectedLegalForm }: HeaderProps) {
  return (
    <header className="mb-8 flex flex-col gap-4 rounded-3xl bg-gradient-to-r from-brand-navy via-brand-violet to-brand-green px-6 py-6 text-white shadow-soft lg:flex-row lg:items-center lg:justify-between">
      <div>
      ...
      <div className="rounded-2xl border border-white/15 bg-white/10 px-4 py-3 backdrop-blur">
        <p className="text-xs uppercase tracking-[0.2em] text-white/60">Pilot-Pfad</p>
        <p className="mt-2 text-sm font-semibold">
          {selectedLegalForm ?? "Noch kein Pilot-Pfad ausgewählt"}
        </p>
      </div>
    </header>
  );
